...
//  判断路径是否可行
int IsPathOk(int graph[][MAX_SIZE],int k,int hami[]);

//  递归的进行哈密顿搜索
bool HamiltonProcess(int graph[][N + 1],int k, int hami[]);
...
int IsPathOk(int graph[][MAX_SIZE], int k, int hami[])
{
    if(visited[hami[k]] == 1)       //  如果当前顶点已经被访问
    {
        //printf("point:%d has been visited\n", hami[k]);
        return 0;
    }

    if(k < N)   //  内部顶点判断是否有路径即可
    {
        //printf("graph[%d][%d] == %d\n", hami[k - 1], hami[k], graph[hami[k-1]][hami[k]]);
		return graph[hami[k-1]][hami[k]];
    }
...
bool HamiltonProcess(int (*graph)[MAX_SIZE],int k, int hami[])
{
    for(hami[k] = 1; hami[k] <= N; hami[k]++)    // 下一个顶点可能是1~N中的任何一个顶点
    {
        /*if(hami[k] == k)
        {
            continue ;
        }*/
    	if(IsPathOk(graph, k, hami) != 0)   //  判断当前是否有边可达
   		{
            ////////////////////////////////////////////////////////////////////
			visited[hami[k]] = true;        // 访问顶点K
            ////////////////////////////////////////////////////////////////////
    		...
    		visited[hami[k]] = false;           //  递归结束后清除标志
            ////////////////////////////////////////////////////////////////////
		}
    }
	return false;
